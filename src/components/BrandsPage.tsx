...
const BrandsPage = () => {
  ...
  const filteredProducts = selectedBrand === 'all' 
    ? products 
    : products.filter(product => product.brand.toLowerCase() === selectedBrand);

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      {/* Hero Section */}
      <div className="relative h-64 bg-gradient-to-r from-purple-600 to-indigo-700 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center">
          <div>
            ...
            <p className="text-xl text-white/90 max-w-2xl">
              Discover premium footwear from the world's leading athletic brands.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 gap-4">
          <div>
            <h2 className="text-2xl font-bold text-slate-900 mb-2">
              All Brands ({filteredProducts.length})
            </h2>
            <p className="text-slate-600">Premium footwear from trusted manufacturers</p>
          </div>
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-12">
            <p className="text-slate-500 text-lg">No products found for this brand.</p>
          </div>
        )}
      </div>
    </div>
